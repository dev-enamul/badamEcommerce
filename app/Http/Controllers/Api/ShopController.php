...
class ShopController extends Controller
{
    public function shop_register(Request $request)
    {
        ...
        if (!$request->has('nid')) {
            return response()->json([
                'success' => false,
                'message' => translate('NID is required.'),
            ], 200);
        }
        ...
        $image_path = public_path('/uploads/doc');
        
        if($request->trade_license == null){
            
            $licenseFileOriginalName = null;
            
        } else {
            
        $trade_license           = $request->file('trade_license');
        $extension               = $trade_license->getClientOriginalExtension();
        $licenseFileOriginalName = $trade_license->getClientOriginalName();
        $trade_license_name      = Str::random(30).'.'.$extension;
        $trade_license->move($image_path, $licenseFileOriginalName); 
        
        }

       if($request->bank_check == null){
            
            $checkFileOriginalName = null;
            
        } else {

        $bank_check            = $request->file('bank_check');
        $extension             = $bank_check->getClientOriginalExtension();
        $checkFileOriginalName = $bank_check->getClientOriginalName();
        $bank_check_name       = Str::random(30).'.'.$extension;
        $bank_check->move($image_path, $checkFileOriginalName);
        ...
        $shop->trade_license = $licenseFileOriginalName;
        $shop->bank_check    = $checkFileOriginalName;
        $shop->slug          = $slug;
