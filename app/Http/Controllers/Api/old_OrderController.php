...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $combined_order->code = 'ORD' . date('Ymdis') . mt_rand(1111, 9999);
        ...
        } else if ($request->payment_type == 'bkash' || $request->payment_type == 'nagad' || $request->payment_type == 'upay' ||  $request->payment_type == 'aamarpay') {
            $go_to_payment = false;

            if ($request->payment_type == 'bkash') {


                $this->paymentDone($combined_order, 'bkash');
            }

            if ($request->payment_type == 'nagad') {

                $this->paymentDone($combined_order, 'nagad');
            }

            if ($request->payment_type == 'upay') {

                $this->paymentDone($combined_order, 'upay');
            }

            if ($request->payment_type == 'aamarpay') {

                $this->paymentDone($combined_order, 'aamarpay');
            }
        } else {
            $go_to_payment = true;
        }
