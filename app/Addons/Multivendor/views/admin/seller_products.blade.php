        ...
        <div class="card-header row gutters-5">
            ...
            <div class="col-md-2 ml-auto">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" name="type" id="type"
                    ...
                    <option value="lowest_price,desc" @isset($col_name, $query) @if ($col_name=='lowest_price' &&
                        $query=='desc' ) selected @endif @endisset>
                        {{ translate('Base Price (High > Low)') }}</option>
                    <option value="highest_price,asc" @isset($col_name, $query) @if ($col_name=='highest_price' &&
                        $query=='asc' ) selected @endif @endisset>
        ...
        <table class="table aiz-table mb-0">
            <thead>
                <tr>
                    ...
                    <th data-breakpoints="md">{{ translate('Publish Product') }}</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{ route('product', $product->slug) }}" target="_blank" class="text-reset d-block">
                    ...
                    <td>
                        <div>
                            <div><span>{{ translate('Rating') }}</span>: <span class="rating rating-sm my-2">{{
                                    renderStarRating($product->rating) }}</span>
                            </div>
                            <div><span>{{ translate('Toal Sold') }}</span>: <span class="fw-600">{{
                                    $product->num_of_sale }}</span></div>
                            <div>
                    ...
                    <td>
                        ...
                        <span class="badge badge-inline badge-md bg-soft-dark mb-1">{{ $category->getTranslation('name')
                            }}</span>
                        @endforeach
                    </td>
                    <td>
                    ...
                    <td>
                        <label class="aiz-switch aiz-switch-success mb-0">
                            <input onchange="update_published(this)" value="{{ $product->id }}" type="checkbox" {{
                                $product->published ==1? 'checked':'unchecked' }}>
...
<script type="text/javascript">
    ...
    function update_published(el) {
        ...
        $.post('{{ route('admin.sellers.product.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status},
        function(data){
            console.log('data', data);
        if(data == 1){
        AIZ.plugins.notify('success', '{{ translate('Product approval successfully') }}');
        }
        else{
